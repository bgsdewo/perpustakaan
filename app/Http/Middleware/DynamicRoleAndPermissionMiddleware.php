...
class DynamicRoleAndPermissionMiddleware
{
    ...
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $routeAccesses = RouteAccess::query()
        ...
        if ($routeAccesses->isNotEmpty()) {
            ...
            foreach ($routeAccesses as $routeAccess) {
                ...
                $role = $routeAccess->role_id
                    ? Role::find($routeAccess->role_id)
                    : null;

                $permission = $routeAccess->permission_id
                    ? Permission::find($routeAccess->permission_id)
                    : null;
