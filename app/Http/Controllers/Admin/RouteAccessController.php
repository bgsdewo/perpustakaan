...
class RouteAccessController extends Controller
{
    public function index(): Response
    {
        ...
        return inertia(component: 'Admin/RouteAccesses/Index', props: [
            'page_settings' => [
                'title' => 'Akses Rute',
                'subtitle' => 'Menampilkan semua data akses rute pada platform ini.',
            ],
            'route_accesses' => RouteAccessResource::collection($route_accesses)->additional(data: [
                'meta' => [
    ...
    {
        return inertia(component: 'Admin/RouteAccesses/Create', props: [
            'page_settings' => [
                'title' => 'Tambah Akses Rute',
                'subtitle' => 'Buat akses rute baru di sini. Klik simpan setelah selesai.',
                'method' => 'POST',
                'action' => route(name: 'admin.route-accesses.store'),
            ],
            ...
            'routes' => collect(value: Route::getRoutes())->map(callback: function ($route) {
                return [
                    'value' => $route->getName(),
                    'label' => $route->getName(),
                ];
